...
let meuGrafico = null;
let chartView = 'daily'; 
let filtrosAtuais = { nome: '', casa: 'todas', tipo: 'todos', status: 'todos' };
let authMode = 'login'; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeApp(isAdmin) {
        ...
        async function loadAdminPanel() {
            const { data: profiles, error } = await supabaseClient.from('profiles').select('id, email');
            if (error) {
        ...
        async function carregarApostas() {
            ...
            if (filtrosAtuais.nome) { query = query.ilike('nome_conta', `%${filtrosAtuais.nome}%`); }
            if (filtrosAtuais.casa && filtrosAtuais.casa !== 'todas') { query = query.eq('casa_apostas', filtrosAtuais.casa); }
            if (filtrosAtuais.tipo && filtrosAtuais.tipo !== 'todos') { query = query.eq('tipo_aposta', filtrosAtuais.tipo); }
            if (filtrosAtuais.status && filtrosAtuais.status !== 'todos') { query = query.eq('status', filtrosAtuais.status); }
            query = query.order('id', { ascending: false });
            
            const { data: apostas, error } = await query;
            if (error) { console.error('Erro ao buscar apostas:', error); return; }

            renderizarLista(apostas);
            atualizarPainelResumo(apostas);
            renderizarGrafico(apostas);
            if (filtrosAtuais.casa === 'todas') { preencherFiltroCasas(apostas); }
        }
        ...
        function renderizarLista(apostas) {
            ...
            if (apostas.length === 0) {
                ...
                apostas.forEach(aposta => {
                    ...
                    apostaElement.innerHTML = `<div class="grid-cell">${aposta.id}</div><div class="grid-cell">${dataFormatada}</div><div class="grid-cell">${aposta.nome_conta || '-'}</div><div class="grid-cell">${aposta.casa_apostas || '-'}</div><div class="grid-cell">${aposta.observacao1 || '-'}</div><div class="grid-cell ${resultadoClasse}">${resultado.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div><div class="grid-cell">${aposta.tipo_aposta || '-'}</div><div class="grid-cell"><span class="status-badge status-${statusClass}">${aposta.status}</span></div><div class="grid-cell action-buttons"><button class="action-btn btn-edit" title="Editar Aposta" data-id="${aposta.id}">✏️</button><button class="action-btn btn-delete" title="Excluir Aposta" data-id="${aposta.id}">🗑️</button></div>`;
        ...
        function renderizarGrafico(apostas) {
            ...
            if (chartView === 'daily') {
                ...
                const labels = mesesOrdenados.map(key => new Date(key.split('-')[0], key.split('-')[1]).toLocaleString('pt-BR', { month: 'short', year: '2-digit' }));
        ...
        function getChartConfig(labels, data, tipoTooltip, mesAtual, maximoY) {
            return { type: 'bar', data: { labels: labels, datasets: [{ label: `Lucro ${tipoTooltip}`, data: data, backgroundColor: 'rgba(250, 204, 21, 1)', borderColor: 'rgba(250, 204, 21, 1)', borderWidth: 1, borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: maximoY, ticks: { color: 'rgba(255, 255, 255, 0.5)' }, grid: { color: 'rgba(255, 255, 255, 0.1)' } }, x: { ticks: { color: 'rgba(255, 255, 255, 0.5)' }, grid: { display: false }, border: { display: false } } }, plugins: { legend: { display: false }, tooltip: { enabled: true, backgroundColor: '#1a1a1a', titleColor: '#fff', bodyColor: '#fff', borderColor: 'var(--primary-color)', borderWidth: 1, displayColors: false, callbacks: { title: (ctx) => tipoTooltip === 'Diário' ? `${ctx[0].label}/${(mesAtual + 1).toString().padStart(2, '0')}` : ctx[0].label, label: (ctx) => `Lucro ${tipoTooltip}: ${ctx.raw.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}` } } } } };
        }
        ...
        async function salvarAposta(apostaData) {
            ...
            if (error) { console.error('Erro ao salvar aposta:', error); alert('Não foi possível salvar a aposta.'); } 
            else { fecharModal(); carregarApostas(); }
        }
        ...
        async function atualizarAposta(apostaData) {
            ...
            if (error) { console.error('Erro ao atualizar aposta:', error); alert('Não foi possível atualizar a aposta.'); } 
            else { fecharModal(); carregarApostas(); }
        }
        ...
        async function excluirAposta(id) {
            if (confirm('Tem certeza que deseja excluir esta aposta?')) {
                ...
                if (error) { console.error('Erro ao excluir aposta:', error); alert('Não foi possível excluir a aposta.'); } 
                else { carregarApostas(); }
            }
        }
        
        async function iniciarEdicao(id) {
            const { data: aposta, error } = await supabaseClient.from('apostas').select('*').eq('id', id).single();
            if (error) { console.error('Erro ao buscar aposta para edição:', error); alert('Não foi possível carregar os dados da aposta.'); return; }
            idApostaEmEdicao = id;
        ...
        function preencherFormulario(aposta) {
            ...
            document.querySelectorAll('input[name="status"]').forEach(radio => { radio.checked = radio.value === aposta.status; });
        }
        ...
        async function salvarNotas() {
            const { data: { user } } = await supabaseClient.auth.getUser();
            if(user) { localStorage.setItem(`notas-${user.id}`, notasTextarea.value); }
        }
        ...
        async function carregarNotas() {
            ...
            if(user) {
                ...
                if (notasSalvas) { notasTextarea.value = notasSalvas; }
            }
        }
        ...
        betsListContainer.addEventListener('click', function(event) {
            ...
            if (target.classList.contains('btn-delete')) { excluirAposta(apostaId); } 
            else if (target.classList.contains('btn-edit')) { iniciarEdicao(apostaId); }
        });
        
        filtroNomeInput.addEventListener('keyup', () => { filtrosAtuais.nome = filtroNomeInput.value; carregarApostas(); });
        filtroCasaSelect.addEventListener('change', () => { filtrosAtuais.casa = filtroCasaSelect.value; carregarApostas(); });
        filtroTipoSelect.addEventListener('change', () => { filtrosAtuais.tipo = filtroTipoSelect.value; carregarApostas(); });
        filtroStatusSelect.addEventListener('change', () => { filtrosAtuais.status = filtroStatusSelect.value; carregarApostas(); });
        limparFiltrosBtn.addEventListener('click', () => {
            filtrosAtuais = { nome: '', casa: 'todas', tipo: 'todos', status: 'todos' };
        ...
        chartDailyBtn.addEventListener('click', () => { chartView = 'daily'; chartDailyBtn.classList.add('active'); chartMonthlyBtn.classList.remove('active'); carregarApostas(); });
        chartMonthlyBtn.addEventListener('click', () => { chartView = 'monthly'; chartMonthlyBtn.classList.add('active'); chartDailyBtn.classList.remove('active'); carregarApostas(); });
        ...
        formNovaAposta.addEventListener('submit', async (event) => {
            ...
            const apostaData = { data: document.getElementById('data').value, nome_conta: document.getElementById('nomeConta').value, observacao1: document.getElementById('obs1').value, observacao2: document.getElementById('obs2').value, tipo_aposta: document.getElementById('tipoAposta').value, casa_apostas: document.getElementById('casaApostas').value, valor_lucro_total_previsto: parseFloat(document.getElementById('lucroTotalPrevisto').value) || 0, status: document.querySelector('input[name="status"]:checked').value, resultado_lucro_total: parseFloat(document.getElementById('resultadoLucroTotal').value) || null };
            if (idApostaEmEdicao) {
